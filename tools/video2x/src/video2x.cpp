...
int single_device_process(Arguments& arguments, video2x::processors::ProcessorConfig& proc_cfg, 
            video2x::encoder::EncoderConfig& enc_cfg, 
            std::shared_ptr<newline_safe_sink> logger_sink)
{
...
int multi_device_process(Arguments& arguments, video2x::processors::ProcessorConfig& proc_cfg, 
            video2x::encoder::EncoderConfig& enc_cfg, 
            std::shared_ptr<newline_safe_sink> logger_sink)
{
   // Create video processor object
    video2x::VideoProcessorMultiDevice video_processor = video2x::VideoProcessorMultiDevice(
        proc_cfg, enc_cfg, arguments.vk_device_list, arguments.hw_device_type, arguments.benchmark, arguments.filter_options
    );
    ...
    while (true) {
        ...
        if (ch == ' ' || ch == '\n') {
            {
                // Toggle pause state
                if (video_processor.get_state() == video2x::VideoProcessorMultiDeviceState::Paused) {
                    video_processor.resume();
                } else {
                    video_processor.pause();
                }
    ...
    // Print final message based on processing result
    if (video_processor.get_state() == video2x::VideoProcessorMultiDeviceState::Aborted) {
        video2x::logger()->warn("Video processing aborted");
        return 2;
    } else if (proc_ret != 0 ||
               video_processor.get_state() == video2x::VideoProcessorMultiDeviceState::Failed) {
        video2x::logger()->critical("Video processing failed with error code {}", proc_ret);
        return 1;
    } else {
        video2x::logger()->info("Video processed successfully");
...
int main(int argc, char** argv) {
    ...
    video2x::logger()->info("multi_device_process");
    return multi_device_process(arguments, proc_cfg, enc_cfg, logger_sink);
}
